...
func NewUdp(messageHandler MessageHandler, localPort int) Udp {
	...
	listener, _ := net.ListenPacket("udp", "0.0.0.0:"+strconv.Itoa(localPort))
	return NewUdpWithConn(messageHandler, listener, localPort)
}

// NewUdpWithConn adopts an already-open socket instead of binding a new one.
// NAT hole punching only works if the punched socket is the one that carries
// the session, and a relay needs to wrap every datagram, so both cases have to
// supply their own PacketConn.
func NewUdpWithConn(messageHandler MessageHandler, conn net.PacketConn, localPort int) Udp {
	u := Udp{}
