...
main.t_itemname = {
	...
	--ONLINE LOBBY
	['lobbybrowser'] = function(t, item)
		main.f_waitForPreloads(true)
		hook.run("main.t_itemname", t, item)
		local m = lobby.f_browser()
		if m ~= nil and m.role == 'spectator' then
			lobby.f_spectate(m)
			exitNetPlay()
			showSessionWarning()
		elseif m ~= nil then
			-- Hole punching only fixes the rollback datagrams; the setup stream
			-- is still TCP, which a host behind NAT cannot accept. Route it
			-- through the lobby when that is on offer. The direct menu entries
			-- below stay direct, since those are for LAN and forwarded ports.
			local relay = lobbyRelayStream()
			local ok
			if m.role == 'host' then
				lobby.hosting = true
				ok = main.f_connect('', gameOption('Netplay.ListenPort'), relay and 'host' or nil)
			else
				ok = main.f_connect(m.hostAddr, m.peerName, relay and 'guest' or nil)
			end
			if ok then
				lobbyMarkPlaying()
				if synchronize() then
...
function main.f_connect(server, str, relayRole)
	if relayRole ~= nil then
		lobbyEnterNetPlay(relayRole == 'host')
	else
		enterNetPlay(server)
	end
	while not connected() do
