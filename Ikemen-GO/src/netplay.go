...
type NetConnection struct {
	ln *net.TCPListener
	// conn is an interface so the match-setup stream can be either a direct
	// TCP link or one spliced through the lobby relay.
	conn             net.Conn
	st               NetState
	sendEnd          chan bool
	recvEnd          chan bool
...
func (nc *NetConnection) Accept(port int) error {
	...
	SafeGo(func() {
		...
		if err := netHandshake(tempConn, true, 2*time.Second); err != nil {
			tempConn.Close()
			return
		}
...
func (nc *NetConnection) Connect(server string, port int) {
	...
	SafeGo(func() {
		...
		for {
			...
			if err := netHandshake(tcpConn, false, 2*time.Second); err != nil {
				tcpConn.Close()
				time.Sleep(100 * time.Millisecond)
				continue
			}
...
// netPlayBegin clears per-session state before either peer role is chosen, so
// the direct and relayed entry points cannot drift apart.
func netPlayBegin() {
	sys.sessionWarning = ""
...
// relayStreamTimeout covers dialling the relay and waiting for the other peer
// to arrive, which can be much later than a direct dial.
const relayStreamTimeout = 45 * time.Second

// netDirectTimeout bounds how long a lobby match tries a plain TCP link
// before falling back to the relay stream. Only worth spending time on when
// hole punching already found a direct UDP path; a NAT strict enough to block
// that will just as surely block a raw TCP accept/dial.
const netDirectTimeout = 3 * time.Second

// AcceptDirectThenRelay listens for a direct connection for a short window,
// then splices the match-setup stream through the lobby relay if nobody
// showed up in time.
func (nc *NetConnection) AcceptDirectThenRelay(port int, plan *RelayPlan, timeout time.Duration) error {
	nc.host = true
	nc.conn = nil
	nc.locIn, nc.remIn = nc.GetHostGuestRemap()
	...
	ln, err := net.Listen("tcp", net.JoinHostPort("", Itoa(port)))
	if err != nil {
		return nc.startRelayed(plan, true)
	}
	tcpLn, ok := ln.(*net.TCPListener)
	if !ok {
		ln.Close()
	...
	SafeGo(func() {
		...
		if err != nil {
			...
			log.Printf("netplay: no direct connection within %s, falling back to the relay", timeout)
			if err := nc.startRelayed(plan, true); err != nil {
				log.Printf("netplay: relay fallback failed: %v", err)
			}
			return
		}
		if nc.isClosing() {
		...
		if err := netHandshake(tempConn, true, 2*time.Second); err != nil {
			tempConn.Close()
			log.Printf("netplay: direct handshake failed, falling back to the relay: %v", err)
			if !nc.isClosing() {
				if err := nc.startRelayed(plan, true); err != nil {
					log.Printf("netplay: relay fallback failed: %v", err)
				}
...
// ConnectDirectThenRelay dials directly for a short window, then falls back
// to the lobby's TCP relay if the host cannot be reached that way.
func (nc *NetConnection) ConnectDirectThenRelay(host string, port int, plan *RelayPlan, timeout time.Duration) error {
	nc.host = false
	nc.conn = nil
	nc.remIn, nc.locIn = nc.GetHostGuestRemap()

	if host == "" || port == 0 {
		return nc.startRelayed(plan, false)
	}

	SafeGo(func() {
		d := net.Dialer{Timeout: timeout}
		tempConn, err := d.Dial("tcp", net.JoinHostPort(host, Itoa(port)))
		if err != nil {
			if nc.isClosing() {
				return
			}
			log.Printf("netplay: direct dial failed, falling back to the relay: %v", err)
			if err := nc.startRelayed(plan, false); err != nil {
				log.Printf("netplay: relay fallback failed: %v", err)
			}
			return
		}
...
// AcceptRelayed and ConnectRelayed replace the listen/dial pair with two
// outbound connections spliced by the lobby, so neither peer needs an inbound
// port. Roles still differ: only the host drives the handshake.
func (nc *NetConnection) AcceptRelayed(plan *RelayPlan) error {
	nc.host = true
	nc.conn = nil
	nc.locIn, nc.remIn = nc.GetHostGuestRemap()
	return nc.startRelayed(plan, true)
...
func (nc *NetConnection) ConnectRelayed(plan *RelayPlan) error {
	nc.host = false
	nc.conn = nil
	nc.remIn, nc.locIn = nc.GetHostGuestRemap()
	return nc.startRelayed(plan, false)
...
func (nc *NetConnection) startRelayed(plan *RelayPlan, sendFirst bool) error {
	conn, err := dialRelayStream(plan, 10*time.Second)
	if err != nil {
		return err
	}
	SafeGo(func() {
		...
		if err := netHandshake(conn, sendFirst, relayStreamTimeout); err != nil {
			log.Printf("netplay: relayed handshake failed: %v", err)
			conn.Close()
			return
		}
		if nc.isClosing() {
...
func (nc *NetConnection) IsConnected() bool {	if nc == nil {
		return false
	}
